...
def _process_table_chunks(
    ...
    """Process table chunks into segments.
    ...
    segments = []
    # Track table count per page for naming: {page_number: table_count}
    ...
    for chunk in table_chunks:
        ...
        try:
            ...
            # Increment table count for this page (1-indexed)
            ...
            # Generate row-level linearizations
            row_results = table_linearizer.linearize_by_row(parsed_table)
            ...
            # Generate cell-level linearizations
            cell_results = table_linearizer.linearize_by_cell(parsed_table)
            # Track row number for cells (cells are generated row by row)
            ...
            for result in cell_results:
                ...
                # Detect row change by row_entity
            ...
            print(f"Error linearizing table: {e}")
            continue
...
def _process_figure_chunks(
    ...
    segments = []
    # Track figure count per page for naming: {page_number: figure_count}
    ...
    for chunk in figure_chunks:
        ...
        # Get figure description from markdown
        description = chunk.get("markdown", "").strip()
        if not description:
            continue

        try:
            # Clean description and transform to claims
            claims = figure_processor.process(description)

            if not claims:
                continue

            # Increment figure count for this page (1-indexed)
